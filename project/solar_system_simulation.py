...
simulation_save_folder = 'solar_system_simulation'
...
data_main_folder = 'solar_system_simulation'
...
planetary_satellite_ephemerides = 'planetary_satellite_ephemerides'
...
satellites_file_name = 'planetary_satellite_physical_parameters.txt'

# Whether planetary satellites will feature in the simulation
LOAD_PLANETARY_SATELLITES = True
# Only the first six satellite ephemerides have already been downloaded in the 
# data folder.

# The satellites are ordered by decreasing GM values in 'satellites_file_name'. 
# If loading satellite data, define the number to skip from the bottom.
skip_num = 42 # satellites from the bottom of the list
# This only includes the first two in the list - Ganymede and Titan, due to 
# computation time.

# For satellites, GM values are listed instead of masses -- but masses required.
...
def simulation_main():
    ...
    if LOAD_PLANETARY_SATELLITES:
        loaded_satellites = sim1.load_physical_params_from_file( 
                data_main_folder, satellites_file_name, lf_usecols=(1,2,3), 
                delimiter='\t', GM_instead_of_mass=True, G_val=G_val, 
                lf_skipfooter=skip_num)
        if loaded_satellites is False:
            print('SIMULATION MAIN: Unable to load satellites.')
            return False
    ...
    if REDOWNLOAD_REQUIRED_DATA:
        ...
        if LOAD_PLANETARY_SATELLITES:
            # Ephemerides for the planetary satellites
            status = sim1.download_ephemerides(loaded_satellites, 
                    data_main_folder, planetary_satellite_ephemerides, 
                    start_time, stop_time, step_size=step_size_api, 
                    t_precision=t_precision, out_units=out_units)
            if status is False:
                print('SIMULATION MAIN: Download of satellite ephemerides failed.')
                return False
    ...
    if LOAD_PLANETARY_SATELLITES:
        ...
        # no load order (but can be defined similarly)
        status = sim1.load_ephemerides(data_main_folder, 
                planetary_satellite_ephemerides, satellite=True)
        if status is False:
            print('SIMULATION MAIN: Loading satellite ephemerides into memory failed.')
            return False

    # 4. Change units if needed
    # -------------------------
    CHANGE_UNITS = False
    if CHANGE_UNITS:
    ...
    # Add additional bodies to the initial conditions (uncomment)
    # -----------------------------------------------------------
    # status = sim1.add_body_to_ICs(id, name, mass, R0, V0, t_initial)
    # if status is False:
    #     print('SIMULATION MAIN: Unable to add body to the initial conditions.')
    #     return False
    ...
    h0=0.0 # first step, default value (step size chosen by integrator)
    hmax=0.0 # max step, default value (step size chosen by integrator)
    ...
    # Calculate the position relative errors of Mars
    # ----------------------------------------------
    id = 399 # Mars Horizons ID
    ...
    # Plot Mars position relative errors
    # ----------------------------------
